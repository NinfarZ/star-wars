        ...
        {vehicle.films.length > 0 && (
          <div className="mb-5">
            ...
            <Suspense fallback={<ItemsSkeleton count={vehicle.films.length} />}>
              <LinkedResourceList<Film> urls={vehicle.films} queryKey="film" routePath="films" getLabel={(film) => film.title} />
            </Suspense>
          </div>
        )}
        ...
        {vehicle.pilots.length > 0 && (
          <div>
            ...
            <Suspense fallback={<ItemsSkeleton count={vehicle.pilots.length} />}>
              <LinkedResourceList<Person> urls={vehicle.pilots} queryKey="person" routePath="people" getLabel={(person) => person.name} />
            </Suspense>
          </div>
        )}
      </section>
